...
const ReviewCard = ({ review }: { review: Review }) => {
    ...
    return (
        <Paper variant="outlined" sx={{
            p: 3, borderRadius: 3, border: `1px solid ${colors.border}`,
            transition: "all 0.2s ease",
            "&:hover": { borderColor: colors.primary, boxShadow: `0 4px 16px ${colors.primaryAlpha10}` },
        }}>
            <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", mb: 2 }}>
                <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
                    <Avatar sx={{ background: gradients.primary, fontWeight: 700, width: 42, height: 42 }}>
                        {initials}
                    </Avatar>
                    <Box>
                ...
                <Box sx={{
                    ...
                    bgcolor: colors.primaryAlpha10, px: 1.5, py: 0.5, borderRadius: 2,
                }}>
            ...
            {review.comment && (
                <Typography variant="body2" color="text.secondary" lineHeight={1.8}
                            sx={{ mb: review.ownerResponse ? 2 : 0 }}>
                    "{review.comment}"
                </Typography>
            )}
...
const ReviewsTab = ({ reviews }: { reviews: Review[] }) => {
    ...
    const avgByCategory = ratingCategories.map(cat => ({
        ...cat,
        avg: reviews.reduce((s, r) => s + r.ratings[cat.key], 0) / reviews.length,
    }));
                ...
                <Box sx={{ display: "flex", flexDirection: { xs: "column", sm: "row" }, alignItems: { sm: "center" }, gap: 3 }}>
                    ...
                    <Box sx={{ textAlign: "center", minWidth: 120 }}>
                        ...
                        <Box sx={{ display: "flex", justifyContent: "center", gap: 0.3, mt: 1 }}>
                            {[1, 2, 3, 4, 5].map(i => (
                                <StarIcon key={i} sx={{ fontSize: 16, color: i <= Math.round(avgOverall / 2) ? "#F59E0B" : colors.primaryAlpha25 }} />
                            ))}
                        </Box>
                        <Typography variant="caption" color="text.secondary">
