...
const Header = () => {
    ...
    // Buton toggle temă — reutilizat în desktop și drawer
    const ThemeToggleButton = () => (
        <Tooltip title={isDark ? "Schimbă la temă luminoasă" : "Schimbă la temă întunecată"}>
            <IconButton
                onClick={toggleMode}
                sx={{
            ...
            <List>
                ...
                {!isAuthenticated && (
                    <Stack spacing={2} sx={{ mt: 4, px: 2 }}>
                        <Button fullWidth variant="outlined"  onClick={() => handleNavigation(paths.login)}>Log In</Button>
                        <Button fullWidth variant="contained" onClick={() => handleNavigation(paths.register)}>Sign Up</Button>
                    </Stack>
                )}
                {/* Toggle temă în drawer */}
                <Box sx={{ mt: 4, px: 2 }}>
                    <Divider sx={{ mb: 2 }} />
                    <ThemeToggleButton />
                </Box>
            ...
            <AppBar elevation={0} sx={{ py: 0.5 }}>
                <Container maxWidth="lg">
                    <Toolbar disableGutters sx={{ justifyContent: "space-between" }}>
                        ...
                        <Stack direction="row" spacing={1} sx={{ display: { xs: "none", md: "flex" }, alignItems: "center" }}>
                                    ...
                                    item.path && (
                                        <Button
                                            ...
                                            sx={{
                                                ...
                                                fontWeight: 700, fontSize: "15px", px: 2, py: 1,
                                                position: "relative",
                            ...
                            {!isAuthenticated ? (
                                <Stack direction="row" spacing={1.5}>
                                    <Button variant="text"      onClick={() => navigate(paths.login)}     sx={{ color: "text.primary", fontWeight: 700 }}>Log In</Button>
                                    <Button variant="contained" onClick={() => navigate(paths.register)}  sx={{ px: 3 }}>Sign Up</Button>
                                </Stack>
                            ) : (
                                <IconButton
                                    onClick={() => logout()}
                                    sx={{
                                        ...
                                        "&:hover": { background: "rgba(192, 57, 43, 0.3)", borderColor: "#96281B", color: "#96281B" },
                                    }}
                                >
                        ...
                        {/* Mobile toggle */}
                        <IconButton onClick={handleDrawerToggle} color="primary" sx={{ display: { md: "none" } }}>
                            <MenuIcon />
                        </IconButton>
                    </Toolbar>
                </Container>
