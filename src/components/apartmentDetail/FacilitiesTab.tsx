...
const FacilitiesTab = ({ facilities }: { facilities: Facilities }) => (
    <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", sm: "1fr 1fr" }, gap: 3 }}>
        {facilityGroups.map((group) => (
            <Paper key={group.title} variant="outlined" sx={{ p: 2.5, borderRadius: 3, border: `1px solid ${colors.border}` }}>
                    ...
                    {group.items.map((item) => {
                        ...
                        return (
                            <Box key={item.key} sx={{ display: "flex", alignItems: "center", gap: 1.5, opacity: active ? 1 : 0.4 }}>
                                <Box sx={{ display: "flex", alignItems: "center", justifyContent: "center", width: 32, height: 32, borderRadius: 2, bgcolor: active ? colors.primaryAlpha10 : "rgba(0,0,0,0.04)", color: active ? colors.primaryDark : colors.textDisabled, "& svg": { fontSize: 18 } }}>
                                    {item.icon}
                                </Box>
                                <Typography variant="body2" fontWeight={active ? 600 : 400} sx={{ color: active ? colors.textPrimary : colors.textDisabled, flex: 1 }}>
                                    {item.label}
                                </Typography>
                                {active ? <CheckIcon sx={{ fontSize: 16, color: colors.success }} /> : <CloseIcon sx={{ fontSize: 16, color: colors.textDisabled }} />}
                            </Box>
                        );
                    })}
                </Box>
