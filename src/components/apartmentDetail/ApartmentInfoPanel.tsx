...
const getIntervalLabel = (i: string) => ({ hour: "oră", day: "zi", month: "lună" }[i] ?? i);
...
const UserCard = ({ user, label, color }: { user: User; label: string; color: string }) => (
    <Card variant="outlined" sx={{ mb: 3, borderRadius: 2, bgcolor: "background.default" }}>
        <CardContent>
            ...
            <Typography variant="body1" color="text.secondary" gutterBottom><strong>Nume:</strong> {user.Name} {user.Surname}</Typography>
            <Typography variant="body1" color="text.secondary" gutterBottom><strong>Email:</strong> {user.Email || "N/A"}</Typography>
            <Typography variant="body1" color="text.secondary"><strong>Telefon:</strong> {user.Phone}</Typography>
        </CardContent>
    </Card>
);
...
const ApartmentInfoPanel = ({ apartment, owner, renter, isAvailable }: Props) => {
    ...
    return (
        <Paper elevation={1} sx={{ p: 4, borderRadius: 4, height: "100%", border: `1px solid ${colors.border}` }}>
            ...
            {owner && <UserCard user={owner} label="Proprietar" color="primary.main" />}
            {!isAvailable && renter && <UserCard user={renter} label="Chiriaș Curent" color="error.main" />}
