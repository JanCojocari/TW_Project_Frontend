...
import { CURRENCY_SYMBOLS, LABELS } from "../../types/paymentPageConfig";
...
const SummaryCard = ({ summary, promoCode, promoMessage, promoDiscount, promoLoading, appliedPromo, onPromoChange, onPromoApply, onPromoRemove }: Props) => {
    ...
    return (
        <Paper elevation={1} sx={{ p: 3, borderRadius: 3, border: `1px solid ${colors.border}` }}>
            <Typography variant="h6" fontWeight={800} mb={2.5} letterSpacing="-0.3px">{LABELS.orderSummary}</Typography>
            ...
            {effectiveDiscount > 0 && <SummaryRow label={LABELS.discount} value={`−${sym} ${effectiveDiscount.toFixed(2)}`} green />}
            <Divider sx={{ my: 1.5 }} />
                ...
                {appliedPromo && promoDiscount > 0 ? (
                    <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", p: 1.5, borderRadius: 2, bgcolor: "rgba(125,170,146,0.1)", border: "1px solid rgba(125,170,146,0.3)" }}>
                        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                            ...
                            <Typography variant="caption" fontWeight={700} color="success.dark">{appliedPromo.toUpperCase()} aplicat</Typography>
                        </Box>
                        <IconButton size="small" onClick={onPromoRemove} aria-label="Elimină codul promo">
                    ...
                    <>
                        <Box sx={{ display: "flex", gap: 1 }}>
                            <TextField size="small" placeholder={LABELS.promoPlaceholder} value={promoCode}
                                       onChange={(e) => onPromoChange(e.target.value.toUpperCase())}
                                       onKeyDown={(e) => e.key === "Enter" && onPromoApply()}
                                       inputProps={{ "aria-label": LABELS.promoPlaceholder, maxLength: 20 }} sx={{ flex: 1 }} />
            ...
            <Box sx={{ display: "flex", alignItems: "center", gap: 1, mt: 2.5, pt: 2, borderTop: `1px solid ${colors.border}`, opacity: 0.65 }}>
                ...
                <Typography variant="caption" color="text.secondary">Plată securizată SSL 256-bit</Typography>
            </Box>
        </Paper>
    );
};
