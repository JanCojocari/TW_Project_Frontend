// theme/theme.ts
// createRentoraTheme(mode) — apelat din ThemeContext cu 'light' | 'dark'

import { createTheme, alpha } from '@mui/material/styles';
import type { PaletteMode }   from '@mui/material';

// ─── Tokeni raw per mod ────────────────────────────────────────────────────
function getRaw(mode: PaletteMode) {
    ...
    return {
        ...
        textPrimary:   isDark ? '#E8EEF5' : '#1F2937',
        textSecondary: isDark ? '#8BA3BE' : '#6B7280',
        textDisabled:  isDark ? '#4A6280' : '#9CA3AF',

        divider: isDark
            ? 'rgba(112, 150, 190, 0.15)'
            : 'rgba(76,  139, 245, 0.12)',

        appBarBg: isDark
            ? 'linear-gradient(90deg, #111D2E 0%, #172540 40%, #1E3050 100%)'
            : 'linear-gradient(90deg, #D8E2EE 0%, #C3D2E5 40%, #8AA8CC 100%)',
        appBarBorder: isDark
            ? 'rgba(112, 150, 190, 0.15)'
            : 'rgba(138, 168, 204, 0.30)',
        appBarShadow: isDark
            ? '0 2px 16px rgba(0, 0, 0, 0.4)'
            : '0 2px 16px rgba(138, 168, 204, 0.25)',
...
// ─── Factory ───────────────────────────────────────────────────────────────
export function createRentoraTheme(mode: PaletteMode) {
    const raw = getRaw(mode);
        ...
        components: {
                ...
                styleOverrides: {
                    root: {
                        '& .MuiOutlinedInput-root': {
                            ...
                            '& fieldset':         { borderColor: raw.divider },
                            '&:hover fieldset':   { borderColor: raw.primaryMain },
                        ...
                        '& .MuiInputBase-input': { color: raw.textPrimary },
                    },
                },
            },
            ...
            MuiIconButton: {
                styleOverrides: {
                    root: {
                        ...
                        '&:hover': { background: alpha(raw.primaryMain, 0.08) },
                    },
                },
            },
            ...
            MuiListItemButton: {
                styleOverrides: {
                    root: {
                        borderRadius: 8,
                        '&:hover': { background: alpha(raw.primaryMain, 0.06) },
                    },
                },
            },
...
// Export raw pentru compatibilitate cu codul existent
export const raw = getRaw('light');
export default createRentoraTheme('light');