...
type RatingKey = "cleanliness" | "location" | "valueForMoney" | "comfort" | "facilities" | "communication";
...
const ReviewCard = ({ review }: { review: Review }) => {
    ...
    const date     = new Date(review.createdAt).toLocaleDateString(i18n.language === "en" ? "en-GB" : "ro-RO", { year: "numeric", month: "long" });
    const stayText = review.stayDuration
        ? ` · ${review.stayDuration} ${review.stayDuration === 1 ? t("components.reviews.day") : t("components.reviews.days")}`
        : "";
            ...
            <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", mb: 2 }}>
                <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
                    ...
                    <Box>
                        ...
                        <Typography variant="caption" color="text.secondary">{date}{stayText}</Typography>
                    </Box>
                </Box>
            ...
            {review.ownerResponse && (
                <Box sx={{ mt: 2, p: 2, bgcolor: colors.primaryAlpha06, borderRadius: 2, borderLeft: `3px solid ${colors.primary}` }}>
                    <Typography variant="caption" fontWeight={700} color="primary.main" sx={{ display: "block", mb: 0.5 }}>
                        {t("components.reviews.ownerResponse")}
                    </Typography>
...
const ReviewsTab = ({ reviews }: { reviews: Review[] }) => {
    ...
    const ratingCategories: { key: RatingKey; label: string }[] = [
        { key: "cleanliness",   label: t("components.reviews.categories.cleanliness")   },
        { key: "location",      label: t("components.reviews.categories.location")      },
        { key: "valueForMoney", label: t("components.reviews.categories.valueForMoney") },
        { key: "comfort",       label: t("components.reviews.categories.comfort")       },
        { key: "facilities",    label: t("components.reviews.categories.facilities")    },
        { key: "communication", label: t("components.reviews.categories.communication") },
    ];
